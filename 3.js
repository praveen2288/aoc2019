class Wire {
  constructor(moves) {
    ...
    this.distanceCoveredAtPoint = [];
  }
  move(direction, distance, startPt) {
    let currentPt = Array.from(startPt);
...
for(let i=0; i<wire1Path.length; i++) {
  for(let j=0; j<wire2Path.length; j++) {
    if ( wire1Path[i][0] == wire2Path[j][0] && wire1Path[i][1] == wire2Path[j][1]  ) {
      console.log('Found intersection at ' + wire1Path[i])
      intersections.push(wire1Path[i]);
    }
  }
}
...
for(let i=0; i<intersections.length; i++) {
  let distance = getManhattanDistance([0,0], intersections[i]);
  console.log('Distance to ' + intersections[i] + ' is ' + distance);
  distances.push(distance);
}

// console.log(Math.min(...distances));
// console.log(WIRE1.distanceCoveredAtPoint);
// console.log(WIRE2.distanceCoveredAtPoint);

let minimumDistances = [];
intersections.forEach(function(intersection) {
  var wire1IntersectionDistance = WIRE1.distanceCoveredAtPoint.filter(function(pt) {
    return pt.point[0] === intersection[0] && pt.point[1] === intersection[1];
  });

  var wire2IntersectionDistance = WIRE2.distanceCoveredAtPoint.filter(function(pt) {
    return pt.point[0] === intersection[0] && pt.point[1] === intersection[1];
  });

  minimumDistances.push(wire1IntersectionDistance[0].distance + wire2IntersectionDistance[0].distance);
});

console.log(Math.min(...minimumDistances));